...
export function useLiveAuction() {
  ...
  const refreshAll = useCallback(async () => {
    const [teamsRes, playersRes, stateRes] = await Promise.all([
      supabase.from('teams').select('id, name, short_code, sponsor_name, captain_name, captain_mobile, captain_photo_url, purse_total, purse_remaining, roster_count, color_hex, logo_url, created_at').order('id'),
  ...
  // v3: the auctioneer picks any player directly (name/serial search)
  // rather than advancing sequentially - this is the primary flow now.
  const loadPlayer = useCallback(async (adminPin, playerId) => {
    const { data, error } = await supabase.rpc('admin_load_player', {
      p_admin_pin: adminPin,
  ...
  // }, []);
    const listTeamPins = useCallback(async (adminPin) => {
      if (!adminPin) return { ok: false, error: 'Admin PIN is required' };
      const { data, error } = await supabase.rpc('admin_list_team_pins', { p_admin_pin: String(adminPin) });
      if (error) return { ok: false, error: error.message };
      const map = {};
      if (data) {
        for (const row of data) map[row.team_id] = row.pin;
